...
// メーカー名の推定（商品名またはJANプレフィックスから）
export function guessMaker(name: string, _janCode?: string): string {
  const lower = name.toLowerCase();
  if (lower.includes('dynabook') || lower.includes('toshiba')) return 'dynabook';
  if (lower.includes('vaio')) return 'VAIO';
  if (lower.includes('thinkpad') || lower.includes('lenovo')) return 'Lenovo';
  if (lower.includes('macbook') || lower.includes('apple')) return 'Apple';
  if (lower.includes('alienware') || lower.includes('dell')) return 'Dell';
  if (lower.includes('hp') || lower.includes('pavilion') || lower.includes('envy')) return 'HP';
  if (lower.includes('asus') || lower.includes('rog') || lower.includes('zenbook')) return 'ASUS';
  if (lower.includes('msi')) return 'MSI';
  if (lower.includes('acer') || lower.includes('aspire') || lower.includes('swift')) return 'Acer';
  if (lower.includes('富士通') || lower.includes('fujitsu') || lower.includes('fmv')) return 'FUJITSU';
  if (lower.includes('nec') || lower.includes('lavie')) return 'NEC';
  if (lower.includes('panasonic') || lower.includes('レッツ') || lower.includes('let\'s')) return 'Panasonic';
  if (lower.includes('sharp') || lower.includes('シャープ')) return 'SHARP';
  if (lower.includes('hitachi') || lower.includes('日立')) return 'HITACHI';
  if (lower.includes('midea')) return 'Midea';
  if (lower.includes('colomo') || lower.includes('kumiai')) return 'その他';
  return '';
}

// CPUの推定（商品名または説明から）
export function guessCpu(text: string): string {
  if (/core\s*i9/i.test(text)) return 'Core i9';
  if (/core\s*i7/i.test(text)) return 'Core i7';
  if (/core\s*i5/i.test(text)) return 'Core i5';
  if (/core\s*i3/i.test(text)) return 'Core i3';
  if (/ryzen\s*9/i.test(text)) return 'Ryzen 9';
  if (/ryzen\s*7/i.test(text)) return 'Ryzen 7';
  if (/ryzen\s*5/i.test(text)) return 'Ryzen 5';
  if (/ryzen\s*3/i.test(text)) return 'Ryzen 3';
  if (/athlon/i.test(text)) return 'Athlon';
  if (/celeron/i.test(text)) return 'Celeron';
  if (/pentium/i.test(text)) return 'Pentium';
  if (/snapdragon/i.test(text)) return 'Snapdragon';
  if (/m1/i.test(text) && /apple|macbook/i.test(text)) return 'Apple M1';
  if (/m2/i.test(text) && /apple|macbook/i.test(text)) return 'Apple M2';
  if (/m3/i.test(text) && /apple|macbook/i.test(text)) return 'Apple M3';
  return '';
}

// SSD容量の推定
export function guessSsd(text: string): string {
  if (/2tb|2000gb|2048gb/i.test(text)) return '2TB';
  if (/1tb|1000gb|1024gb/i.test(text)) return '1TB';
  if (/512gb|500gb/i.test(text)) return '512GB';
  if (/256gb|250gb/i.test(text)) return '256GB';
  if (/128gb/i.test(text)) return '128GB';
  if (/64gb/i.test(text)) return '64GB';
  return '';
}

// メモリ容量の推定
export function guessMemory(text: string): string {
  if (/32\s*gb.*メモリ|メモリ.*32\s*gb|32gb\s*メモリ/i.test(text)) return '32GB';
  if (/16\s*gb.*メモリ|メモリ.*16\s*gb|16gb\s*メモリ/i.test(text)) return '16GB';
  if (/8\s*gb.*メモリ|メモリ.*8\s*gb|8gb\s*メモリ/i.test(text)) return '8GB';
  if (/4\s*gb.*メモリ|メモリ.*4\s*gb|4gb\s*メモリ/i.test(text)) return '4GB';
  return '';
}

// JANコード・商品名から全スペックを一括推定
export function autoParseSpecs(name: string, description: string, janCode?: string): {
  maker: string; cpu: string; ssd: string; memory: string;
} {
  const combined = `${name} ${description}`;
  return {
    maker: guessMaker(name, janCode),
    cpu: guessCpu(combined),
    ssd: guessSsd(combined),
    memory: guessMemory(combined),
  };
}
